...
app.post('/new/song', function(req, res) {
    ...
    songFile.mv('/var/www/html/uploads/songs/'+req.body.fb_id+rString+'.mp3', function(err) {
        if (err) {
        	...
        	var song  = { 
					  ...
					  href: "http://139.59.1.150/uploads/songs/"+req.body.fb_id+rString+".mp3",
					  created_at: moment().format('YYYY-MM-DD HH:mm:ss')
			    };
			console.log(song);
		...
		con.query("SELECT liked_id FROM right_swipes WHERE liker_id = ? ", req.query.fb_id  , function (error, liked, fields) {
			...
			var x=0;
				...
				con.query("SELECT * FROM users WHERE fb_id NOT IN ("+arr+")", function (error, results, fields) {
		     	...
		     	var resultsObjArr = JSON.parse(resultsString);
				console.log(resultsObjArr);		
				res.status(200).send(resultsObjArr);
				});
